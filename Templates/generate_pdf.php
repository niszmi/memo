...
//            $signature = 'data:image/jpg;base64,' . base64_encode(file_get_contents('../' . $signaturePath));
//        }

        // Check if a new signature file was uploaded
        if (isset($_FILES['signature_' . $i]) && $_FILES['signature_' . $i]['error'] == 0) {
            $signaturePath = '../signatures/' . basename($_FILES['signature_' . $i]['name']);

            // Move the uploaded file to the desired directory
            if (move_uploaded_file($_FILES['signature_' . $i]['tmp_name'], $signaturePath)) {
                ...
                $signature = null; // Handle the error if the file couldn't be moved
            }
        } else {
            // If no new file was uploaded, use the existing signature path from the database
            $existingSignaturePath = '../' . $_POST["existing_signature_$i"] ?? null;
            if ($existingSignaturePath && file_exists($existingSignaturePath)) {
                $mimeType = mime_content_type($existingSignaturePath);
                $signature = 'data:' . $mimeType . ';base64,' . base64_encode(file_get_contents($existingSignaturePath));
            } else {
                $signature = null;
            }
        }
...
//                $mimeType = mime_content_type($signaturePath);
//
//                // Check if the MIME type is an image and supported
...
//        }
        ...
        if ($name && $jawatan) {
            $signatures[] = [
                ...
                'jawatan' => $jawatan,
                'date' => $date,
                'catatan' => $catatan, // Include catatan
                'signature' => $signature,
                'sk' => $sk, // Include SK
...
//        echo "<pre>Signature " . ($index + 1) . " base64 data:</pre>";
//        echo "<pre>" . htmlentities($signatureData['signature']) . "</pre>";
//    }
//
//    exit(); // Stop execution so you can check the output
//
...
<div class="container mt-4">
    <div class="header-logo">
     ...
     <h1 style="text-align: center; margin-top: 10px">MEMO</h1>
...
$html .= <<<EOD
        <div class="signature-block" style="margin-top: 10px">
            <img src="{$signature['signature']}" alt="Signature" style="width: 120px;">
            <div style="font-weight: bold; font-size:11pt; margin-top: 5px; text-transform: uppercase;">({$signature['name']})</div>
        <table style="width: 100%; margin-top: 5px; border-collapse: collapse;">
            <tr>
            <td style="width: 70%; text-align: left; text-transform: capitalize;">{$signature['jawatan']}</td>
EOD;
